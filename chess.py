...
class ChessGame(object):
    ...
    def __find_non_pawn(self, move, to, piece):
        ...
        if p == 'r':
            ...
            return next(s for s, pt in self.state.items()
                if pt == piece and key in s and self.__check_knight_move(s, to))
        else:
            return next(s for s, pt in self.state.items()
                if pt == piece and key in s
                and (self.__check_line(s, to) or self.__check_diagonal(s, to)))
    ...
    def __find_pawn(self, move):
        ...
        if len(move) == 2:
            if self.is_white_turn:
                origin = c + next(str(i) for i in range(r, 0, -1)
                    if self.state[c + str(i)] == pt)
            else:
                origin = c + next(str(i) for i in range(r, 9)
                    if self.state[c + str(i)] == pt)
        else:
            if self.state[move[-2:]] != '':
                origin = move[0] + (str(r - 1) if self.is_white_turn else str(r + 1))
            else: # En passant
